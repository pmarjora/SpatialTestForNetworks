...
(FindNeighbors(net,5))
(FindNeighbors(net,10))
(FindNeighbors(net,15))
```

Smoothing the labels to make them correlated...
```{r, smoother}
...
Here, we use George's idea to generate data $y = (I_n − \rho W)^{−1} \times \epsilon$, where $\epsilon \sim MVN(0,I_n)$.
× ", "  MV N(0, In)
```{r}
SARsmoother
# Simulating a SAR model with only y
n <- 10
rho <- .5 # Spatial autocorrelation
set.seed(12315)
# Bernoulli graph
W <- matrix(as.integer(runif(n*n)<.25), ncol = n)
diag(W) <- 0
...
We suppose that each node has some (binary or continuous) annotationxi, and standardize those values by setting $z_i=x_i−\bar{x}$. The LISA measure of local clustering for each node, $i$, is then defined as 
