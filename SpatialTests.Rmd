...
Calcluating Moran's (global) I. Moran's-I is defined as \[ I=\frac{N}{W}\frac{\sum_i \sum_j w_{ij} (x_i-\bar{x})(x_j-\bar{x})}{\sum_i(x_i-\bar{x})^2} \]
```{r, MoransI}
...
Smoothing the labels to make them correlated. Here we use a simple proof of principle scheme in ewhich we generate the labels independently and then smooth them by taking a weight average of each label and the label of its neighboring vertices. Later on, we will try something more formal.
```{r, smoother}
